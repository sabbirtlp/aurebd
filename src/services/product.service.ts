...
async function _getFeaturedProducts() {
  await dbConnect();
  return await Product.find({}).select("name slug price discountPrice image category stock isNewArrival isBestSeller isSpecialOffer isGiftSet soldCount").limit(8).lean();
}
